...
#import <HHAttachmentSheetView/HHAttachmentSheetViewDefine.h>

@interface HHAttachmentMenuCell ()
{
...
@implementation HHAttachmentAssetCell {
    HHCheckboxButton *_checkButton;
...
- (void)setAsset:(TGMediaAsset *)asset signal:(SSignal *)signal
{
    ...
    if (self.selectionContext != nil)
    {
        if (_checkButton == nil)
        {
            _checkButton = [[HHCheckboxButton alloc] initWithStyle:HHCheckboxButtonStyleMedia];
        ...
        __weak HHAttachmentAssetCell *weakSelf = self;
        [_itemSelectedDisposable setDisposable:[[self.selectionContext itemInformativeSelectedSignal:(id<TGMediaSelectableItem>)asset] startWithNext:^(TGMediaSelectionChange *next)
                                                {
                                                    __strong HHAttachmentAssetCell *strongSelf = weakSelf;
                                                    if (strongSelf == nil)
                                                        return;
    ...
    [signal startWithNext:^(id next)  {
        ...
        void (^block) (void) = ^ {
            if ([next isKindOfClass:[UIImage class]]) {
                [strongSelf->_imageView setImage:next];
            }
        };
